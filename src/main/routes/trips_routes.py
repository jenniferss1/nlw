...
from src.models.repositories.trip_repository import TripsRepository
...
def link_finder(tripId):
    ...
    response = controller.create(tripId)
    return jsonify(response["body"]), response["status_code"]
...
def get_trip_participants(trupId):
    conn = db_connection_handler.get_connection()
    participants_repository = ParticipantsRepository(conn)
    controller = ParticipantFinder(participants_repository)
    
    response = controller.find_participants_from_trip(trupId)
    return jsonify(response["body"]), response["status_code"]
